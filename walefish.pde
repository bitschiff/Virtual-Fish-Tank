class walefish extends fish
{

  walefish() {
    PVposition.set(random(500) + 51, random(500) + 51);
    fweight=(randomGaussian() * (.25*20))+20;
    fmaxWeight=(randomGaussian() * (.25*100))+100;
    fage=0;
    fmaxAge=(randomGaussian() * (.25*30000))+30000;
    String names[] = loadStrings("names.txt");
    String[] m = match(names[floor(random(names.length))], "(.*?),");
    fname=m[0] + " whale";
    fDead=false;
    fr=(randomGaussian() * (.25*150))+150;
    fg=fr;
    fb=fr;
    fcanHaveBaby=false;
    PVmove.set((randomGaussian() * (.25*(sqrt(1)/2)))+(sqrt(1)/2), 0);
    if (random(2)>1)
  ...
  void move() {
    ...
    if (floor(random(15)) == 0 && fDead == false)
    {
      if (floor(random(12)) == 0)
        PVmove.set(-1*PVmove.x, PVmove.y);
    }
